...
def _hparams(algorithm, dataset, random_seed):
    ...
    lf_alg = 'entropy'
    if algorithm == "DRM":
        _hparam('lf_alg', lf_alg, lambda r: r.choice(['entropy', 'nn', 'cosine']))
        _hparam('prototype_K', 50, lambda r: r.choice([1, 5, 10, 20]))
        
    if algorithm in ['ERMNP']:
        _hparam('temperature', 0.1, lambda r: 10**r.uniform(-2, 2))
        if dataset in SMALL_IMAGES:
            _hparam('lr', 0.001, lambda r: 10**r.uniform(-4.5, -2.5))
            _hparam('queue_size', 1000, lambda r: 10**r.uniform(-2, 2))
            _hparam('k', 10, lambda r: 10**r.uniform(-2, 2)) # top-k most similar samples
        else:
            _hparam('queue_size', 1000, lambda r: 10**r.uniform(-2, 2))
            _hparam('k', 50, lambda r: 10**r.uniform(-2, 2)) # top-k most similar samples
    ...
    if algorithm in ['DANN', 'CDANN', 'DANN_test'] or (algorithm=='DRM' and lf_alg=='nn'):
        ...
        if dataset in SMALL_IMAGES:
            _hparam('lambda', 0.5, lambda r: 10**r.uniform(-2, 2))# lambda for best: 0.5
            _hparam('weight_decay_d', 0.0, lambda r: 10**r.uniform(-6, -2))
            _hparam('d_steps_per_g_step', 1, lambda r: int(2**r.uniform(0, 3)))
            _hparam('grad_penalty', 1.0, lambda r: 10**r.uniform(-2, 1))
            _hparam('beta1', 0.5, lambda r: r.choice([0., 0.5]))
            _hparam('mlp_width', 512, lambda r: int(2 ** r.uniform(6, 10)))
            _hparam('mlp_depth', 5, lambda r: int(r.choice([3, 4, 5])))
            _hparam('mlp_dropout', 0.0, lambda r: r.choice([0., 0.1, 0.5]))
        else:
            _hparam('lambda', 0.5, lambda r: 10**r.uniform(-2, 2))
            _hparam('weight_decay_d', 1e-5, lambda r: 10**r.uniform(-6, -2))
            _hparam('d_steps_per_g_step', 5, lambda r: int(2**r.uniform(0, 3)))
            _hparam('grad_penalty', 1.0, lambda r: 10**r.uniform(-2, 1))
            _hparam('beta1', 0.5, lambda r: r.choice([0., 0.5]))
            _hparam('mlp_width', 256, lambda r: int(2 ** r.uniform(6, 10)))
    ...
    elif algorithm == "IRM" or algorithm == "DIRM":
        _hparam('irm_lambda', 2.73, lambda r: 10**r.uniform(-1, 5))
        _hparam('irm_penalty_anneal_iters', 8,
    ...
    elif algorithm == "MMD" or algorithm == "CORAL" or algorithm == "DMMD" or algorithm == "DCORAL":
    ...
    if dataset in SMALL_IMAGES and algorithm in ['IRM']:
    ...
    if algorithm in ['DANN', 'CDANN', 'DANN_test'] and dataset in SMALL_IMAGES:
        _hparam('lr_g', 1e-3, lambda r: 10**r.uniform(-4.5, -2.5))
    elif algorithm in ['DANN', 'CDANN', 'DANN_test']:
        _hparam('lr_g', 5e-5, lambda r: 10**r.uniform(-5, -3.5))

    if algorithm in ['DANN', 'CDANN', 'DANN_test', 'DRM'] and dataset in SMALL_IMAGES:
        _hparam('lr_d', 1e-3, lambda r: 10**r.uniform(-4.5, -2.5))
    elif algorithm in ['DANN', 'CDANN', 'DANN_test', 'DRM']:
